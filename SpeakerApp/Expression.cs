...
namespace SpeakerApp
{
	...
	public class Expression : SRZBaseVisitor<object>
	{
		...
		public override object VisitOpNotExp([NotNull] SRZParser.OpNotExpContext context)
		{
			return !Convert.ToBoolean(Visit(context.expression()));
		}
		...
		public string LeftPadByZero(int num) => String.Format("{0:d2}", num);

		public string CombineFullMaketName(string maket, int number, int elem, int exam)
			=> String.Format("{0:s2}{1:d2}{2:d2}{3:d2}", maket, number, elem, exam);

		public override object VisitVariable(SRZParser.VariableContext context)
        {
            object variable;
			// если экземпляр макета не указан, то по умолчанию используем первый, иначе указанный
			var exam = context.exam == null ? 1 : Int32.Parse(context.exam.Text);
			// макет должен выглядеть как то так в итоге: [имя макета:1][номер макета:2][номера показателя:2][номер экземпляра:2]
			var varname = context.name.Text.ToLowerInvariant();//.PadRight(7, '0')+LeftPadByZero(exam);
			//var varname = CombineFullMaketName(maket, number, elem, exam);
			
			if (!DataRepository.TryGetValue(varname, out variable)) 
